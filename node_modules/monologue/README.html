...
</style><title>README</title></head><body><article class="markdown-body"><h1 id="monologue-streamlined-query-building"><a name="user-content-monologue-streamlined-query-building" href="#monologue-streamlined-query-building" class="headeranchor-link" aria-hidden="true"><span class="headeranchor"></span></a>Monologue - Streamlined query building</h1>
<p><a href="https://nodei.co/npm/monologue/"><img alt="NPM" src="https://nodei.co/npm/monologue.png?compact=true" /></a></p>
<p><strong>Security Notice &amp; Breaking Changes</strong></p>
<ul>
<li>
<p>In previous versions, monologue did not automatically escape column names (or table names), only values.  So unless you were using <code>monologue().backquote()</code>, your queries may be at risk.  Column names and table names are now automatically backquoted, so check your query output to make sure it doesn&rsquo;t break anything.  This can be disabled by passing an option like this: <code>monologue({backquote: false})</code>.</p>
</li>
<li>
<p>Bound parameter-style queries have been removed since in 0.5.0.  Documentation has been removed for quite a while, so this should affect very few, if any, users.</p>
</li>
<li>
<p>In previous versions, when doing multiple inserts(array of objects) the object keys were sorted alphabetically.  Instead of doing this automatically, it is now optional and defaults to not sorting.  This will make the output of the query more predictable based on input.  If you would like to enable sorting, you can pass <code>monologue({sort_keys: true})</code> as an option.  It is not necessary to sort keys, monologue puts your insert statements in the correct order (based on the order of the first object in the collection), but the option is there for unit testing compatibility and for analyzing output in testing.</p>
</li>
</ul>
<p><strong>New features</strong></p>
<p>New methods were added for doing different types of comparison. File a github issue if you have some feedback, maybe they&rsquo;re stupid/useless, you be the judge:</p>
<pre><code>// new methods: .gt(), .lt(), .gte(), .lte(), and .not()
...
<p><a href="https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&amp;hosted_button_id=9KXDJTKMBPXTE">Support Development</a></p>
...
<p>This area needs a ton of work.  You can get some great examples in the section by the same name.  For now, here&rsquo;s a quick rundown of <code>monologue().backquote()</code>:</p>
<pre><code>// result: [ '`email`', '`password`', '`type`' ]
...
<p><em>These are a little out of date due to recent changes but are pretty close to correct</em></p>
<pre><code>var monologue = require('monologue');
